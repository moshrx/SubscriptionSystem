...
const { subscriptions,getDeactivatedApps, addSubscription, updateSubscription, getApps, deactivateSubscription, getDashboardData } = require('../controllers/subscriptionController');
...
router.get('/deactivated-apps/:userId', getDeactivatedApps);

//api to get subscription count to check for basic or premium user
router.get('/subscriptionCount', async (req, res) => {
    const userId = req.query.userId;
    try {
        const subscriptionCount = await Subscription.countDocuments({ userId, inActive: false});
...
router.get('/subscriptions/:userId', async (req, res) => {
    try {
        ...
        const subscriptions = await Subscription.find({ userId, inActive: false }); // Query by userId
